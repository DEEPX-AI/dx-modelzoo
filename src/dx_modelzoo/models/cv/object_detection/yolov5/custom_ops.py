"""YOLOv5 output decoding variants."""
...
@POSTPROCESSING_REGISTRY.register("yolov5_decode")
class YOLOv5Decode:
    """Standard YOLOv5 decode: [1, N, 5+C] → obj_conf × cls → NMS input."""
...
@POSTPROCESSING_REGISTRY.register("yolov5_multiscale_decode")
class YOLOv5MultiscaleDecode:
    """YOLOv5 multi-scale anchor decode: raw [B, 3, H, W, 85] per scale."""
    ...
    def __call__(self, outputs, **kwargs):
        ...
        for scale_idx in range(num_scales):
            ...
            B, na_dim, H, W, nc = output.shape
            output = output.transpose(0, 2, 3, 1, 4)  # [B, H, W, 3, 85]
        ...
        decoded = np.concatenate(all_preds, axis=1)[0]  # remove batch → [N, 5+C]
