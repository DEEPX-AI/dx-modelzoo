...
@EVALUATOR_REGISTRY.register("instance_segmentation")
class InstanceSegmentationEvaluator(EvaluatorBase):
    """COCO Evaluator for instance segmentation (mAP)."""
    ...
    def process_batch_result(self, batch_data: Tuple, output: Any, metrics_state: dict) -> dict:
        ...
        # Postprocessing returns (detections[M,6], masks[M,H,W]) tuple
        if isinstance(output, (list, tuple)) and len(output) == 2:
            outputs, final_masks = output
        else:
            outputs, final_masks = output, None
    ...
    def _make_coco_format_seg(self, masks: np.ndarray, outputs: np.ndarray, img_id: int) -> List[dict]:
        ...
        masks_hwn = np.asfortranarray(masks.transpose(1, 2, 0))
        rles = rle_encode(masks_hwn)
        ...
        for i in range(len(outputs)):
            ...
            category_id = (
                ...
                else self.coco80_to_91_map.get(int(output[5]), int(output[5]))
            )
            rles[i]["counts"] = rles[i]["counts"].decode("utf-8")
    ...
    def _run_coco_eval(self) -> Tuple[float, float]:
        try:
            ...
            logger.error("Error during COCO segmentation evaluation: {}", e)
            return 0.0, 0.0
