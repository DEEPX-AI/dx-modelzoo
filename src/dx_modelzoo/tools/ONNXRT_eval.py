    ...
    for task_name, model_names in model_list_by_task.items():
        for model_index, model_name in enumerate(model_names):
            ...
            try:
                ...
                model =  ModelFactory(model_name, SessionType.onnxruntime,model_path, data_dir).make_model()
...
def evaluate_models_internal(model_names: List[str],
    ...
    for model_name in tqdm(model_names, desc="Evaluating models on GPU or CPU"):
        logger.info(f"@JSON <START Evaluation> [{model_name}]")
        ...
        try:
            ...
            ## onnx file location
            possible_paths = [
                f'/mnt/modelzoo_storage/{name_parts[1]}-{name_parts[2]}/raw.onnx',
                f'/mnt/modelzoo_storage/{name_parts[1]}-{name_parts[2]}/dynamic.onnx',
                f'/mnt/modelzoo_storage/{name_parts[1]}-{name_parts[2]}/{name_parts[1]}-{name_parts[2]}.onnx'
            ]
            ...
            if model_path is None:
                raise FileNotFoundError(
                    f"Neither raw.onnx or dynamic.onnx or {name_parts[1]}-{name_parts[2]}.onnx found for model {model_name} "
                    f"in directory /mnt/modelzoo_storage/{name_parts[1]}-{name_parts[2]}/"
                )
...
def make_model_path_S3(model_name):
    ...
    os.makedirs(f"{MODEL_LOCAL_PATH}/onnx/", exist_ok=True)
    # download models from amazon server.
    url = f"https://sdk.deepx.ai/modelzoo/onnx/{public_name}.onnx"
    model_local_path = f"{MODEL_LOCAL_PATH}/onnx/{public_name}.onnx"
...
#     model_list_by_task = \
#             {'image_classfication':['ResNet18', 'ResNet34', 'ResNet50', 'ResNet101', 'ResNet152', 
...
#                                     'RegNetX400MF', 'RegNetX800MF', 'RegNetY200MF', 'RegNetY400MF', 'RegNetY800MF', 'ResNeXt50_32x4d', 'ResNeXt26_32x4d', 
#                                     'DenseNet121', 'DenseNet161','HarDNet39DS', 'WideResNet50_2', 'WideResNet101_2', 
#                                     'OSNet0_25', 'OSNet0_5', 'RepVGGA1'],
#             'object_detection':['YoloV3', 'YoloV5N', 'YoloV5S', 'YoloV5M', 'YoloV5L', 'YoloV7', 'YoloV7E6', 'YoloV7Tiny', 'YoloV8L', 'YoloV9C', 'YoloV9S', 'YoloV9T', 'YoloXS',
#                                 'SSDMV1', 'SSDMV2Lite'],
#             'face_id':['YOLOv5s_Face', 'YOLOv5m_Face', 'YOLOv7s_Face', 'YOLOv7_Face', 'YOLOv7_TTA_Face', 'YOLOv7_W6_Face', 'YOLOv7_W6_TTA_Face'],
#             'semantic_segmentation':['DeepLabV3PlusMobilenet',  'DeepLabV3PlusDRN', 'DeepLabV3PlusMobileNetV2', 'DeepLabV3PlusResNet101', 'DeepLabV3PlusResNet50', 
#                                     'DeepLabV3PlusResnet', 'BiSeNetV1', 'BiSeNetV2',],
#             'image_denoising':['DnCNN_15', 'DnCNN_25', 'DnCNN_50']}
    
#     local_model_list_path = "/mnt/regression_storage/accuracy_modelzoo_data/M1A/2"
#     folder_name_list = os.listdir(local_model_list_path)
#     do_public_name_list = []
#     for folder_name in folder_name_list:
#         name_parts = extract_folder_name(folder_name)
#         if name_parts:
#             do_public_name_list.append(name_parts[0])
        
#     evaluate_models(
#         input_model_list=do_public_name_list,
#         local_model_list_path=local_model_list_path,
