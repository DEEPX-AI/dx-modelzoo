...
show_help() {
    ...
    echo "Options:"
    echo "  --dxrt_src_path=<path>         Set DXRT source path (default: ../dx-runtime/dx_rt)"
...
validate_path() {
    ...
    if [ ! -d "$DX_AS_PATH" ]; then
        echo -e "${TAG_INFO} ${COLOR_BRIGHT_YELLOW_ON_BLACK}[Normal mode]${COLOR_RESET}"
        echo -e "${TAG_INFO} ${COLOR_BRIGHT_YELLOW_ON_BLACK}[DXRT_SRC_PATH=${DXRT_SRC_PATH}]${COLOR_RESET}"
    else
        echo -e "${TAG_INFO} ${COLOR_BRIGHT_YELLOW_ON_BLACK}[DX-AS mode] (DX_AS_PATH: ${DX_AS_PATH} is detected)${COLOR_RESET}"
        echo -e "${TAG_INFO} ${COLOR_BRIGHT_YELLOW_ON_BLACK}[DXRT_SRC_PATH=${DXRT_SRC_PATH}]${COLOR_RESET}"
    fi
    ...
    if [ ! -d $DXRT_SRC_PATH ]; then
        local err_msg="${COLOR_BRIGHT_RED_ON_BLACK}DXRT_SRC_PATH ($DXRT_SRC_PATH) does not exist.\nUse the '--dxrt_src_path=<dir>' option to specify the directory where the 'dx_rt' source code is located.${COLOR_RESET}"
...
install_python() {
    ...
    echo -e "Checking if Python $REQUIRED_MAJOR.$MIN_REQUIRED_MINOR or newer is installed..."

    found_suitable_python=false
    installed_version_message=""

    if command -v python3 &> /dev/null; then
        echo -e "Checking version of 'python3' command..."
        CURRENT_PYTHON_VERSION_FULL=$(python3 --version 2>&1)
        CURRENT_PYTHON_VERSION_STRING=$(echo -e "$CURRENT_PYTHON_VERSION_FULL" | awk '{print $2}' | sed 's/[^0-9.]*//g')
        ...
        if [[ -n "$CURRENT_PYTHON_VERSION_STRING" && "$CURRENT_PYTHON_VERSION_STRING" =~ ^[0-9]+\.[0-9]+ ]]; then
            ...
            echo -e "Detected 'python3' points to version: $CURRENT_MAJOR.$CURRENT_MINOR"
            ...
            if [ "$CURRENT_MAJOR" -eq "$REQUIRED_MAJOR" ] && [ "$CURRENT_MINOR" -ge "$MIN_REQUIRED_MINOR" ]; then
                ...
                echo -e "'python3' is version $CURRENT_MAJOR.$CURRENT_MINOR, which is older than required."
            fi
        else
            echo -e "Could not parse version from 'python3 --version' output: $CURRENT_PYTHON_VERSION_FULL"
        fi
    else
        echo -e "'python3' command not found."
    fi

    if ! $found_suitable_python; then
        echo -e "Checking for specific Python versions (python$REQUIRED_MAJOR.$MIN_REQUIRED_MINOR onwards)..."
    ...
    if $found_suitable_python; then
        echo -e "$installed_version_message, which satisfies the requirement of Python $REQUIRED_MAJOR.$MIN_REQUIRED_MINOR or newer."
        echo -e "Skipping Python $TARGET_INSTALL_VERSION installation."
        return 0
    else
        echo -e "No suitable Python version ($REQUIRED_MAJOR.$MIN_REQUIRED_MINOR or newer) found through 'python3' or specific 'python3.X' commands."
    fi

    echo -e "Installing Python $TARGET_INSTALL_VERSION..."
    if sudo add-apt-repository -y ppa:deadsnakes/ppa; then
        ...
        if sudo apt-get install -y "python${TARGET_INSTALL_VERSION}" "python${TARGET_INSTALL_VERSION}-dev" "python${TARGET_INSTALL_VERSION}-venv"; then
            echo -e "Python $TARGET_INSTALL_VERSION installation process completed successfully."
        else
            echo -e "${TAG_ERROR} Failed to install Python $TARGET_INSTALL_VERSION packages."
            return 1
        fi
    else
        echo -e "${TAG_ERROR} Failed to add PPA ppa:deadsnakes/ppa."
        return 1
    fi
}
...
setup_venv() {
    ...
    if grep -qE "/docker|/lxc|/containerd" /proc/1/cgroup || [ -f /.dockerenv ]; then
        CONTAINER_MODE=true
        echo -e "(container mode detected)"
