"""Tests for dx_modelzoo.dataset.objectron."""

import pytest

from dx_modelzoo.dataset.objectron import Objectron


class TestObjectron:
    def test_import(self):
        """Verify the class is importable and registered."""
        assert Objectron is not None

    def test_init_with_fake_path(self):
        """Verify __init__ doesn't crash with a fake path (no file access yet)."""
        try:
            ds = Objectron("/nonexistent/path")
    ...
    def test_is_dataset_base(self):
        ...
        assert issubclass(Objectron, DatasetBase)
