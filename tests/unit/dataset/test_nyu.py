"""Tests for dx_modelzoo.dataset.nyu."""

import pytest

from dx_modelzoo.dataset.nyu import NYUDepthv2


class TestNYUDepthv2:
    def test_import(self):
        """Verify the class is importable and registered."""
        assert NYUDepthv2 is not None

    def test_init_with_fake_path(self):
        """Verify __init__ doesn't crash with a fake path (no file access yet)."""
        try:
            ds = NYUDepthv2("/nonexistent/path")
    ...
    def test_is_dataset_base(self):
        ...
        assert issubclass(NYUDepthv2, DatasetBase)
